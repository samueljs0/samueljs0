...
def wrap(value: str, max_chars: int = 46) -> list[str]:
    words = value.split(" ")
    lines, current = [], ""
    for w in words:
...
def build_svg() -> str:
    ...
    for label, value in ROWS:
        label = escape(label)
        value_lines = [escape(line) for line in wrap(value)]
        begin = f"{delay:.2f}s"
        rows_svg.append(f"""
  <g class="row" transform="translate(24,{y})" opacity="0">
    ...
    <text class="value" x="120" y="0">{value_lines[0]}</text>
  </g>""")
        y += LINE_HEIGHT
  ...
  <g class="row" transform="translate(24,{y})" opacity="0">
    <animate attributeName="opacity" from="0" to="1" dur="0.5s" begin="{begin}" fill="freeze"/>
    <text class="value" x="120" y="0">{extra}</text>
  </g>""")
            y += LINE_HEIGHT
        delay += 0.18
